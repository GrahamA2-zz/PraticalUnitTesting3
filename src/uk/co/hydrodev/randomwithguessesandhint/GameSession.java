...
public class GameSession {
	...
	public String makeAGuess(int myNumber) {
		...
		if( guessCount <   ( maxGuesses - 1 )  ){
			guessCount++;
			if ( generator.getMyNumber() == myNumber ){  //18 After adding and testing my new class I can refactor this test
				gameRunning = false;
				return CORRECT;
			} else {
				guesses.add(myNumber);
				//16.	Report the hint solution
				String hint = myNumber > generator.getMyNumber() ? "lower" : "higher"; 
				return String.format(WRONG,maxGuesses - guessCount, prettyPrint(), hint ); //3.	Reformat the wrong response the include number of guesses
			}
		}
		else {
			gameRunning = false;
